...
{
    public class DeltaJsonSerializerDataContractResolver : IDataContractResolver
    {
        private readonly JsonSerializerOptions serializerOptions;

        public DeltaJsonSerializerDataContractResolver(JsonSerializerOptions serializerOptions)
        {
        ...
        private IEnumerable<object> GetSerializedEnumValuesFor(Type enumType)
        {
            ...
            var serializeAsString = underlyingValues.Any()
                && JsonSerializer.Serialize(underlyingValues.First(), this.serializerOptions).StartsWith("\"");

            return serializeAsString
                ? underlyingValues.Select(value => JsonSerializer.Serialize(value, this.serializerOptions).Replace("\"", string.Empty))
                : underlyingValues;
        }
        ...
        private IEnumerable<DataProperty> GetDataPropertiesFor(Type objectType, out Type extensionDataValueType)
        {
            ...
            var applicableProperties = objectType.GetProperties(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                .Where(property =>
                {
                    return
                        ...
                        !(this.serializerOptions.IgnoreReadOnlyProperties && !property.IsPubliclyWritable());
                });
            ...
            foreach (var propertyInfo in applicableProperties)
            {
                ...
                var name = propertyInfo.GetCustomAttribute<JsonPropertyNameAttribute>()?.Name
                    ?? this.serializerOptions.PropertyNamingPolicy?.ConvertName(propertyInfo.Name) ?? propertyInfo.Name;
